...
public class CoachControllerImpl implements CoachController {
    ...
    @Override
    @GetMapping
    public PagedModel<?> getAllCoaches(@RequestParam(defaultValue = "0") int page,
    ...
    @Override
    @GetMapping("/{id}")
    public EntityModel<CoachDto> getCoachById(@PathVariable Long id) throws CoachNotFoundException {
        return coachService.getCoachById(id);
    }

    @Override
    @PostMapping
    public EntityModel<CoachDto> createCoach(@RequestBody CoachDto newCoach) {
        return coachService.createCoach(newCoach);
    }

    @Override
    @PutMapping("/{id}")
    public EntityModel<CoachDto> updateCoach(@PathVariable Long id, @RequestBody CoachDto newCoach) throws CoachNotFoundException {
